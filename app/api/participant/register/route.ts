...
import { validateParticipantProfileInput } from "../../../lib/participant-profile";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const profileValidation = validateParticipantProfileInput(body.profile);
    if (!profileValidation.ok) return jsonError(400, profileValidation.error);
    ...
    const account = await insertAccount({
      ...
      email,
      username,
      roleCode: PARTICIPANT_ROLE,
      password: await hashPassword(password),
      createdBy: "yarışmacı kaydı",
      mustChangePassword: false,
      participantProfile: profileValidation.value,
    });
