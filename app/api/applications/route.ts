...
import { competitionAcceptsApplications, createApplication, findCompetitionWorkflow, findLatestProfileForCompetition, listApplications, listOpenCompetitions, reportBucket } from "../../lib/workflow-db";
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    // kapalı bir yarışma hiç seçilemez. Yetki kararı değildir, POST'taki kontrol yerinde.
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    // Yarışma adının YETKİLİ kaynağı yayımlanmış profildir, koddaki sabit havuz değil:
    // şartnameden çıkarılan ad (ör. bir festival adı) havuzda bulunmayabilir ve
    // bu kontrol yayımlanmış yarışmayı başvuruya kapatıyordu.
    ...
    // Blob doğrudan verilir: ReadableStream ile R2 uzunluğu bilemediği için
    // yükleme sessizce düşebiliyordu. `File` zaten bir `Blob`.
