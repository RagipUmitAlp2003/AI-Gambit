...
export async function markApplicationAnalyzing(id: string, judge: AdminAccount): Promise<"started" | "profile_missing" | "conflict"> {
   ...
   * kabul ediliyordu. Başarılı bir analizden sonra durum 'awaiting_judge'
   * olduğu için hakem ekranındaki "Analizi yenile" düğmesi sunucuda 409
   * ("zaten analiz edilmiş") ile reddediliyordu: kriterler güncellendiğinde
   * ya da sonuç şüpheli göründüğünde yeniden analiz yapılamıyordu.
   *
   * Artık hakem kuyruğundaki (analiz edilmiş) dosya da yeniden analiz
   * edilebilir. Kesinleşmiş karar ('completed') ve kararları dondurulmuş
   * yarışma korumaları YERİNDE: onlar için önce "Kararı yeniden aç" gerekir
   * ve bu kural `saveApplicationEvaluation` içinde ayrıca uygulanır.
   */
