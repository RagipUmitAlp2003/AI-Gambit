...
export async function saveApplicationReview(id: string, judge: AdminAccount, review: JudgeReview): Promise<void> {
  ...
  if (!completed && storedStamp && (review.draftSavedAt ?? null) !== storedStamp) {
    throw new ConflictError(
  ...
  review = { ...review, draftSavedAt: completed ? null : timestamp };
...
export async function attachSimilarityToEvaluation(
  ...
  ).bind(next, new Date().toISOString(), id, current.evaluation_json).run();
