...
export const adminApi = {
  ...
  flows: () => request<{ flows: DocumentFlow[]; viewerRole: RoleCode }>("/api/admin/flows"),

  createFlow: (input: DocumentFlowInput) =>
    request<{ flow: DocumentFlow }>("/api/admin/flows", { method: "POST", body: JSON.stringify(input) }),

  updateFlow: (id: string, input: DocumentFlowInput) =>
    request<{ flow: DocumentFlow }>(`/api/admin/flows/${id}`, { method: "PATCH", body: JSON.stringify(input) }),

  deleteFlow: (id: string) => request<{ deleted: boolean }>(`/api/admin/flows/${id}`, { method: "DELETE" }),
};
