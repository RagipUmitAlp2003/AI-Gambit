...
/** Panelde tek seferde yalnızca bir görünüm açılır; kaydırma yerine sekme ile geçilir. */
type PanelView = "create" | "accounts" | "outbox";
...
export default function AdminAccountsPanel({ accounts, roles, mailReady, mail, viewerId, onChanged }: Props) {
  ...
  return (
    <section className="workspace" aria-labelledby="accounts-title">
      ...
      <nav className="admin-view-nav" aria-label="Panel bölümleri">
        <button
          type="button"
        ...
        >
          <strong>Yeni hesap ata</strong>
          <small>Form ve tek kullanımlık şifre</small>
        </button>
        <button
          type="button"
        ...
        >
          <strong>Kayıtlı yönetici hesapları</strong>
          <small>{accounts.length} kayıt · {activeCount} aktif</small>
        </button>
        <button
          type="button"
          className={view === "outbox" ? "active" : ""}
          aria-pressed={view === "outbox"}
          onClick={() => setView("outbox")}
        >
          <strong>Bildirim kayıtları</strong>
          <small>{mail.length} bildirim</small>
        </button>
      </nav>
            ...
            <div className="form-actions">
              <span className="save-note">
                {mailReady
                  ...
                  : "Mail sağlayıcı tanımlı değil: bildirim giden kutusuna alınır; Bildirim kayıtları sekmesinden görülür."}
              </span>
